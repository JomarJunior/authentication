...
from src.Shared.Config.AppConfig import AppConfig, OSType
...
class TestAppConfig:
    ...
    def test_init_with_valid_data(self, valid_config_data):
        ...
        assert config.appName == "TestApp"
        assert config.version == "1.0.0"
        assert config.debug is False
        assert config.databaseUrl == "postgresql://localhost:5432/test"
        assert config.port == 8080
        assert config.host == "0.0.0.0"
        assert config.logTarget == "console"

    def test_operating_system_default(self, valid_config_data):
        """Test operating system is set correctly and is a valid OSType."""
        config = AppConfig(**valid_config_data)
        assert isinstance(config.operatingSystem, OSType)
        assert config.operatingSystem in [OSType.WINDOWS, OSType.LINUX, OSType.MACOS, OSType.OTHER]
    ...
    def test_default_values(self, temp_dir):
        ...
        assert config.debug is False
        assert config.databaseUrl == "sqlite:///:memory:"
...
class TestAppConfigFromEnv:
    ...
    def test_from_env_with_all_env_vars(self, temp_dir):
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            ...
            assert config.appName == "EnvApp"
            assert config.version == "2.1.3"
            assert config.debug is True
            assert config.databaseUrl == "postgresql://env:5432/envdb"
            assert config.port == 9000
            assert config.host == "env.example.com"
            assert config.logTarget == "file"
    ...
    def test_from_env_with_defaults(self, temp_dir):
        ...
        default_path = "/tmp/Authentication"
        os.makedirs(default_path, exist_ok=True)

        with patch.dict(
            os.environ,
            {"FILESYSTEM_PATH": temp_dir, "DATABASE_URL": "sqlite:///:memory:"},
            clear=True,
        ):
            config = AppConfig.FromEnv()

            assert config.appName == "Authentication"
            assert config.version == "1.0.0"
            assert config.debug is False
            assert config.databaseUrl == "sqlite:///:memory:"
            assert config.port == 8000
...
class TestAppNameValidator:
    ...
    def test_invalid_app_name_characters_raise_error(self, base_config_data):
        ...
        for name in invalid_names:
            ...
            with pytest.raises(ValidationError) as exc_info:
                AppConfig(**base_config_data)
            assert "must contain only alphanumeric characters and underscores" in str(
                exc_info.value
            )
...
class TestAppConfigIntegration:
    ...
    def test_complete_config_creation_and_validation(self, temp_dir):
        ...
        config_data = {
            ...
            "version": "1.2.3",
            "debug": True,
            "databaseUrl": "postgresql://localhost:5432/Authentication",
            "port": 8080,
            "host": "0.0.0.0",
            "logTarget": "file",
        }
        ...
        assert config.debug is True
        assert config.databaseUrl == "postgresql://localhost:5432/Authentication"
        assert config.port == 8080
        assert config.host == "0.0.0.0"
        assert config.logTarget == "file"
        assert isinstance(config.operatingSystem, OSType)
    ...
    def test_from_env_integration(self, temp_dir):
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            ...
            assert config.appName == "Authentication_Test"
            assert config.version == "2.0.1"
            assert config.debug is True
            assert config.databaseUrl == "postgresql://test:5432/test_db"
            assert config.port == 9090
            assert config.host == "test.localhost"
            assert config.logTarget == "both"
